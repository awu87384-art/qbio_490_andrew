...
#Question 4
colnames(clinic)
clinic$age_at_initial_pathologic_diagnosis[5]
```
...
```{r Exercise 4.1}
x <- as.numeric(clinic$age_at_diagnosis)
y <- as.numeric(clinic$last_contact_days_to)
...
```{r Exercise 4.2}
age <- as.numeric(clinic$age_at_diagnosis)
gender <- clinic$gender

good <- is.finite(age) & !is.na(gender)

boxplot(age[good] ~ gender[good],
...
The y-axis represents the survival probability, or the proportion of patients still alive over time.
...
The Middle age group appears to have the highest survival probability overall.
# 4. What age group appears to have the worst rate of survival? 
The Young age group appears to have the lowest survival probability.
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
15 years is about 5,475 days. At that time point, the Old group’s survival probability is approximately 30%.
...
The p-value is 0.2. This is not statistically significant (p > 0.05), meaning there is no strong evidence that survival differs significantly between age groups in this dataset.
